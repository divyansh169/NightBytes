...
public class DeliveryShipOrderView extends AppCompatActivity {
    ...
    LinearLayout l1;
    public static String deliddd = FirebaseAuth.getInstance().getCurrentUser().getUid() ;
...
//        DatabaseReference databaseRegference = FirebaseDatabase.getInstance().getReference("deliveryshipper").child((FirebaseAuth.getInstance().getCurrentUser().getUid())).child("suburbanus");
//        databaseRegference.addListenerForSingleValueEvent(new ValueEventListener() {
//            @Override
//            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
//                Type9 type9 = dataSnapshot.getValue(Type9.class);
//                suburbin = type9.getsuburbanus();

//                deliddd = FirebaseAuth.getInstance().getCurrentUser().getUid();

        reference = FirebaseDatabase.getInstance().getReference("DeliveryShipFinalOrders").child(deliddd).child(RandomUID).child("Dishes");
...
//});
        ...
        DatabaseReference databaseReference = FirebaseDatabase.getInstance().getReference("DeliveryShipFinalOrders").child(deliddd).child(RandomUID).child("OtherInformation");
