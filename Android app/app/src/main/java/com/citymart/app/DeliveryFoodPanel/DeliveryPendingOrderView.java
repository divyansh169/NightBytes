...
public class DeliveryPendingOrderView extends AppCompatActivity {
    ...
    LinearLayout l1;
    public static String delividd= FirebaseAuth.getInstance().getCurrentUser().getUid();
...
//        DatabaseReference databaseReferencge = FirebaseDatabase.getInstance().getReference("deliveryshipper").child((FirebaseAuth.getInstance().getCurrentUser().getUid())).child("suburbanus");
//        databaseReferencge.addListenerForSingleValueEvent(new ValueEventListener() {
//            @Override
//            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
//                Type9 type9 = dataSnapshot.getValue(Type9.class);
//                suburbin = type9.getsuburbanus();
//                delividd = FirebaseAuth.getInstance().getCurrentUser().getUid();

        reference = FirebaseDatabase.getInstance().getReference("DeliveryShipOrders").child(delividd).child(RandomUID).child("Dishes");
...
//});
        ...
        DatabaseReference databaseReference = FirebaseDatabase.getInstance().getReference("DeliveryShipOrders").child(delividd).child(RandomUID).child("OtherInformation");
...
//                address.setText((deliveryShipOrders1.getAddress()).substring(22));
                address.setText((deliveryShipOrders1.getAddress()).substring((deliveryShipOrders1.getAddress()).indexOf(':') + 1).trim());
